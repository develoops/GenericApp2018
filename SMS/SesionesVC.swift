//
//  SesionesVCViewController.swift
//  SMS
//
//  Created by Arturo Sanhueza on 09-10-18.
...
class SesionesVC: UIViewController,UITableViewDelegate,UITableViewDataSource{
    
    @IBOutlet weak var tabla: UITableView!
    var dateFormatter = DateFormatter()
    var tamanoCelda = CGFloat()
    var charlasPersona:[PFObject]!
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        queryPersona.findObjectsInBackground().continueWith{ (taskPersona:BFTask<NSArray>) -> Any? in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let evento = charlasPersona[indexPath.row] as PFObject
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let evento = charlasPersona[indexPath.row]
        var object = Array<Any>()
        _ = personas.map{if($0.value(forKey:"act") as? PFObject == evento){
            let persona = $0.value(forKey: "persona") as? PFObject
        ...
        vc.evento = evento
        print(personas)
        vc.personas = object as! [PFObject]
        if(evento["tipo"] as? String == "conferencia")
        {
